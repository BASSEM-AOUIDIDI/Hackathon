...
/*
	...
	*/
	function logMessage(username, timestamp, url) {
		var msg= username+" accessed the site "+url+" at "+timestamp+".";
...
/*
    ...
    */
    function substringBetweenMatches(str, subStr) {
    	var firstmatch = str.indexOf(subStr);
    	var lastmatchOf=str.lastIndexOf(subStr)
    	var between=str.slice(firstmatch+3,lastmatchOf)
    	return between;
    }
...
/*
	...
	*/
	function both_ends(count) {
		if (count.length<2)
			return ''
		count =count.slice(0,2) + count.slice(-2)
		return (count) ;
	}
...
/*
	...
	*/
	function mix_up(a, b) {
	mix1= b.slice(0,2)+a.charAt(2)
	mix2= a.slice(0,2)+b.charAt(2)
	return (mix1, mix2);
}
...
/*
	...
	*/
	function not_bad(str) {
		...
		var good='good'
		if (n > 0 && n<b ) {
			var res = str.slice(n, b+3);
			var newstr=str.replace(res, good)
			return newstr
			}
		else return str	
